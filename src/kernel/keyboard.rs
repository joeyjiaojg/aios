...
fn decode_scancode(scancode: u8) -> Option<KeyEvent> {
    ...
    Some(KeyEvent { scancode: code, pressed: true })
}
...
mod tests {
    ...
    fn test_keyboard_buffer_push_pop() {
        ...
        let event = KeyEvent { scancode: 0x10, pressed: true };
    ...
    fn test_keyboard_buffer_full() {
        ...
        for _ in 0..KEYBOARD_BUFFER_SIZE {
            assert!(buf.push(KeyEvent { scancode: 0, pressed: true }));
        }
        assert!(!buf.push(KeyEvent { scancode: 0, pressed: true }));
    }
